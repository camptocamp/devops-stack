...
// TODO Clarify what to put on the scope
. The commit scope is optional but recommended:
.. At least, for the modules that have variants, *it is recommended to add the variant on the scope (`eks`, `aks` or `kind`)*. It could simply use the variant or use the variant as a prefix (for example, `docs(eks-variables): add descriptions`).
