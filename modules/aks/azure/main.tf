...
module "argocd" {
  ...
  cert_manager = {
    ...
    node_pool = try(coalesce([for k, v in var.node_pools : contains(v.argo_apps, "cert-manager") ? k : ""]...), null)
  }
  traefik = {
    # retrieve node pool name where component should be deployed
    node_pool = try(coalesce([for k, v in var.node_pools : contains(v.argo_apps, "traefik") ? k : ""]...), null)
  }
